...
import { ResponsiveContainer, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, LineChart, Line, FunnelChart, Funnel, LabelList } from 'recharts';
      ...
      <Card>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={400}>
            <BarChart data={data.levelDistribution} layout="horizontal">
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis type="number" />
              <YAxis dataKey="level" type="category" width={80} />
              <Tooltip 
                formatter={(value, name) => [
                  `${value} empresas (${data.levelDistribution.find(d => d.companies === value)?.percentage.toFixed(1)}%)`,
                  'Empresas'
                ]}
              />
              <Bar 
